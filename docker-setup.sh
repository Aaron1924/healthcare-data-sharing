#!/bin/bash
# Docker setup script for healthcare data sharing project
...
if ! command -v docker &> /dev/null; then
  ...
  print_error "You can also try running: sudo apt-get update && sudo apt-get install -y docker.io"
  exit 1
fi
...
if ! command -v docker-compose &> /dev/null; then
  ...
  print_error "You can try running: sudo apt-get install -y docker-compose"
  print_error "Or use Docker Compose plugin: sudo apt-get install -y docker-compose-plugin"
  exit 1
fi
...
if ! docker info &> /dev/null; then
  ...
  print_error "If you're using Docker Desktop, make sure it's running."
  print_error "If you're using Linux, try: sudo systemctl start docker"
  exit 1
fi
...
init_ipfs() {
  print_message "Initializing IPFS..."
  ./init-ipfs.sh
}

# Function to start the containers
start_containers() {
  # Check if IPFS data directory exists
  if [ ! -d "ipfs-data" ]; then
    print_message "IPFS data directory not found. Initializing IPFS first..."
    init_ipfs
  fi
  ...
  if [ $? -eq 0 ]; then
    ...
    print_message "  IPFS Gateway: http://localhost:8080"
    print_message "  IPFS WebUI: http://localhost:5001/webui"
    ...
    print_message "  IPFS Gateway: http://$LOCAL_IP:8080"
    print_message "  IPFS WebUI: http://$LOCAL_IP:5001/webui"
    print_message ""
    print_message "For more information on external access, see EXTERNAL_ACCESS.md"
  else
...
stop_containers() {
  ...
  if [ $? -eq 0 ]; then
    ...
    print_error "Failed to stop containers. Check the logs for more information."
    exit 1
  fi
}
...
cleanup() {
  print_warning "This will remove all containers, volumes, and local data. Are you sure? (y/n)"
  read -r confirm
  if [[ $confirm == "y" || $confirm == "Y" ]]; then
    print_message "Removing containers, volumes, and local data..."
...
create_sample_data() {
  print_message "Creating sample data..."
  docker-compose exec api python -m backend.create_sample_records
  print_message "Sample data created successfully!"
}

# Function to show help
show_help() {
  echo "Healthcare Data Sharing Docker Setup Script"
  echo ""
  ...
  echo "  cleanup     Remove all containers, volumes, and local data"
  echo "  samples     Create sample data"
  echo "  init        Initialize IPFS configuration (run this first if you have IPFS issues)"
